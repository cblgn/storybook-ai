...
démarrent FastAPI et Vite et utilisent un `TestModel` PydanticAI déterministe.
Ils couvrent le proxy, les champs, l’envoi, les résultats, l’attente et la reprise
après erreur, sur ordinateur et mobile. Libérer les ports 8000 et 5173 avant
`pnpm test:e2e`. Les modèles de test ne sont activés que dans `backend/tests`.

`pnpm test` lance Vitest sans navigateur externe ; `pnpm test:coverage` ajoute
le rapport `frontend/coverage/lcov.info`. Les tests de composants simulent la
frontière HTTP et vérifient le formulaire, l’attente, les erreurs, la reprise et
l’affichage du résultat. pytest-cov produit `backend/coverage.xml` et échoue sous
80 % de couverture. Les rapports générés restent hors de Git. Les futurs tests
de fournisseur réel doivent porter le marqueur `integration`, exclu par défaut,
et être invoqués explicitement avec `pytest -m integration`.
